...
Generated code is not checked (`RuleTiers.GeneratedCodeIsExempt`). A file Roslyn classifies as
generated, such as `*.g.cs` or one headed `// <auto-generated/>`, carries its generator author's
decisions rather than yours, so no rule asks you about them, with one exception: DD0008 reports a
suppression of a DD rule wherever it is, generated files included.
...
| Set | Decides |
| --- | --- |
| [`rule-tiers`](https://github.com/Hafeok/decision-driven-analyzers/blob/main/docs/decisions/rule-tiers.md) | What a rule is, its tier, what it ships with, and what it does not look at |
