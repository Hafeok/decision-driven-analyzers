...
public sealed class SuppressionTests
{
    ...
    [Fact]
    public void A_packages_content_file_under_the_package_root_is_not_reported()
    {
        // The case a consumer met: a test framework compiles helper sources from its package into
        // every test project, headed <auto-generated>. They are the package author's code, in the
        // package cache, and the consumer can neither edit nor delete the header.
        string root = PackageRoot();
        string path = System.IO.Path.Combine(root, "sample.testing.core", "4.0.1", "_content", "DefaultResultWriters.cs");

        ImmutableArray<Diagnostic> diagnostics = RuleHarness.RunOn(
            new SuppressionAnalyzer(),
            Compile(path, "// <auto-generated/>" + Environment.NewLine + LookAwaySource),
            WithPackageRoot(BuildProperties(ProjectDirectory()), root));

        Assert.Empty(diagnostics);
    }

    [Fact]
    public void A_hand_written_file_with_the_package_root_known_is_still_reported()
    {
        // Knowing the package root exempts files under it and nothing else.
        string project = ProjectDirectory();
        string path = System.IO.Path.Combine(project, "DefaultResultWriters.cs");

        ImmutableArray<Diagnostic> diagnostics = RuleHarness.RunOn(
            new SuppressionAnalyzer(),
            Compile(path, "// <auto-generated/>" + Environment.NewLine + Empty),
            WithPackageRoot(BuildProperties(project), PackageRoot()));

        Assert.Single(diagnostics);
    }

    [Fact]
    public void A_file_under_the_package_root_is_reported_when_the_root_is_not_known()
    {
        // Without the property the rule does not guess what a package cache looks like.
        string path = System.IO.Path.Combine(PackageRoot(), "sample.testing.core", "4.0.1", "_content", "DefaultResultWriters.cs");

        ImmutableArray<Diagnostic> diagnostics = RuleHarness.RunOn(
            new SuppressionAnalyzer(),
            Compile(path, "// <auto-generated/>" + Environment.NewLine + Empty),
            BuildProperties(ProjectDirectory()));
    ...
    /// <summary>What NuGet hands over: an absolute directory with a trailing separator.</summary>
    private static string PackageRoot() =>
        System.IO.Path.Combine(System.IO.Path.GetTempPath(), "home", ".nuget", "packages") + System.IO.Path.DirectorySeparatorChar;

    private static Dictionary<string, string> WithPackageRoot(Dictionary<string, string> properties, string root)
    {
        properties["build_property.NuGetPackageRoot"] = root;
        return properties;
    }
