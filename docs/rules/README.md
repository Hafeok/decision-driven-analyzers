...
(`rule-tiers.RuleDeliverableOrder`). Start from [`_template.md`](_template.md).
...
| --- | --- |
| **Id** | `DDnnnn`. Generic rules are `DD0001` onward (`two-packages.RuleIdPrefixDD`). An id is never reused, even after a rule is removed. |
| **Tier** | 1, 2 or 3, with the severity that follows from it. Fixed when the rule is agreed, not when it is written (`rule-tiers.ThreeTiers`). |
...
| **Motivating decision** | `<Set>.<Key>` — the set id from the front matter of a file in `docs/decisions/`, and the decision's key. Exactly the string the analyzer cites. A rule with no decision behind it is an opinion, and does not ship. |
...
- **Tier 2** — heuristic. Ships as a warning, and the false-positive story is the section
  ...
  (`rule-tiers.Tier2PromotionBySupersession`).
- **Tier 3** — needs the whole graph and so cannot be a diagnostic at all. It lives in
  ...
  (`rule-tiers.Tier3NeverGatesWithoutBaseline`).
